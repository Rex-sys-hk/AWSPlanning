...
class Policy(GaussianMixin, Model):
    ...
    def compute(self, inputs, role):
        ...
        x = self.net(inputs["states"].view(-1, *self.observation_space.shape))
        return 1 * torch.tanh(x), self.log_std_parameter, {}   # AGV action_space is -1 to 1 # 
...
class Value(DeterministicMixin, Model):
    ...
    def compute(self, inputs, role):
        # view (samples, width * height * channels) -> (samples, width, height, channels)
        # permute (samples, width, height, channels) -> (samples, channels, width, height)
        return self.net(inputs["states"].view(-1, *self.observation_space.shape)), {}


# Load and wrap the JetBot environment (a subclass of Gym)
env = AGVEnv(skip_frame=1, headless=False)
env = wrap_env(env)
