...
class AWS_PID:
    def __init__(self) -> None:
        self.vx_pid = PID(1, 0.01, 0.1, setpoint=0)
        self.vx_pid.output_limits = (-0.5, 0.5)
        self.vy_pid = PID(1, 0.01, 0.1, setpoint=0)
        self.vy_pid.output_limits = (-0.5, 0.5)
        self.omega_pid = PID(1, 0.01, 0.1, setpoint=0, error_map=pi2pi)
        self.omega_pid.output_limits = (-0.5, 0.5)
        self.pids = [self.vx_pid, self.vy_pid, self.omega_pid]

    def __call__(self, state, dt, set_point=None) -> Any:
        controls = []
        if state is None or len(state) != len(self.pids):
            raise ValueError('state must be a list of length {}'.format(len(self.pids)))
        if set_point is not None and len(set_point) == len(self.pids):
            for i, pid in enumerate(self.pids):
                pid.setpoint = set_point[i] if set_point[i] is not None else pid.setpoint
        for i, pid in enumerate(self.pids):
            controls.append(pid(state[i], dt))
        return controls
