...
void Smoother::tracePath(const Node3D* node, int i, std::vector<Node3D> path) {
  ...
  // std::cout << "node count: " << i << std::endl;
  // std::cout << "node address: " << node << std::endl;
  // std::cout << "node pred address: " << node->getPred() << std::endl;
  // std::cout << "node: " << node->getX() << " " << node->getY() << " " << node->getT() << std::endl;
  i++;
  path.push_back(*node);
  if (node->getPred() == node || i > 3000){
    std::cout << "looping in retrieve" << std::endl;
    this->path = path;
    return;
  }
