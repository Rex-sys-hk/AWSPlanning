...
Planner::Planner() {
  ...
  subGetState = n.subscribe("/odom", 1, &Planner::CB_getRobotState, this);
};
...
//  std::cout << "created Voronoi Diagram in ms: " << d * 1000 << std::endl;
  ...
  if (!Constants::manual && listener.canTransform("/world", ros::Time(0), "/body", ros::Time(0), "/world", nullptr)) {
...
void Planner::setStart(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& initial) {
  ...
  float t = Helper::normalizeHeadingRad(tf::getYaw(initial->pose.pose.orientation));
  // publish the start without covariance for rviz
  geometry_msgs::PoseStamped startN;
  startN.pose.position = initial->pose.pose.position;
  startN.pose.orientation = initial->pose.pose.orientation;
  startN.header.frame_id = "world";
...
void Planner::CB_getRobotState(const nav_msgs::Odometry& odom)
{
  if (!Constants::online_test) return;
  if (odom_received) return;
  std::cout<<"CB_getRobotVel"<<std::endl;
  std::cout<<"==odom==: \n "<<odom.twist.twist<<std::endl;
  // TODO: add start velocity to the start node
  start_odom = odom;
  // start_vel = odom.twist.twist;
...
void Planner::setGoal(const geometry_msgs::PoseStamped::ConstPtr& end) {
  ...
  float t = Helper::normalizeHeadingRad(tf::getYaw(end->pose.orientation));
  ...
  if (grid->info.height >= y && y >= 0 && grid->info.width >= x && x >= 0) {
    ...
    if (Constants::manual && !Constants::online_test) { plan();}

  } else {
    std::cout << "invalid goal x:" << x << " y:" << y << " t:" << Helper::toDeg(t) << std::endl;
  }
  if (Constants::online_test) {
    tf::StampedTransform transform;
    try{
      listener.lookupTransform("/world", "/body",  
                               ros::Time(0), transform);
      geometry_msgs::PoseStamped startN;
      startN.pose.position.x = transform.getOrigin().x();
      startN.pose.position.y = transform.getOrigin().y();
      tf::quaternionTFToMsg(transform.getRotation(), startN.pose.orientation);
      // startN.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(
      ...
      startN.header.frame_id = "world";
      startN.header.stamp = ros::Time::now();
      // TODO: set odom_received to false after the goal is reached
      float x = transform.getOrigin().x()/ Constants::cellSize;
      float y = transform.getOrigin().y() / Constants::cellSize;
      float t = Helper::normalizeHeadingRad(transform.getRotation().z());
      std::cout << "I am seeing a new start x:" << x << " y:" << y << " t:" << Helper::toDeg(t) << std::endl;
      ...
      if (grid->info.height >= y && y >= 0 && grid->info.width >= x && x >= 0) {
        ...
        geometry_msgs::PoseWithCovarianceStamped start_odom;
        start_odom.pose.pose.position.x = transform.getOrigin().x();
        start_odom.pose.pose.position.y = transform.getOrigin().y();
        tf::quaternionTFToMsg(transform.getRotation(), start_odom.pose.pose.orientation);
        // start_odom.pose.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(
        // transform.getRotation().x(),
        // transform.getRotation().y(),
        // transform.getRotation().z()
        // );
...
void Planner::plan() {
  ...
  if (validStart && validGoal) {
    ...
    NodeAWS* nodes3D_aws = new NodeAWS[length]();
    ...
    if (Constants::online_test && odom_received)
    {
      nStart_aws.setVX(start_odom.twist.twist.linear.x);
      nStart_aws.setVY(start_odom.twist.twist.linear.y);
      nStart_aws.setVYaw(start_odom.twist.twist.angular.z);
    }
    if (nStart_aws.getVX() == 0. && nStart_aws.getVY() == 0.){
      nStart_aws.setVX(1e-2);
    }
    std::cout<<"nStart_aws vx,cy,vyaw: "<<nStart_aws.getVX()<<", "<<nStart_aws.getVY()<<" "<<nStart_aws.getVYaw()<<std::endl;
    // ___________
    // DEBUG START
    ...
    smoothedPath.clear();
    // FIND THE PATH
    std::cout << "I am planning..." << std::endl;
    Node3D* nSolution;
    if (Constants::use_aws)
      nSolution = Algorithm::hybridAStar(nStart_aws, nGoal_aws, nodes3D_aws, nodes2D, width, height, configurationSpace, dubinsLookup, visualization);
    else
    ...
    std::cout<<"nSolution: "<<nSolution<<std::endl;
    TracePath(nSolution, 0, path_p);
    std::cout<<"path_p.size(): "<<path_p.size()<<std::endl;
    path.updatePath(path_p);
    // SMOOTH THE PATH
    // if (Constants::path_smooth) smoother.smoothPath(voronoiDiagram);
    // CREATE THE UPDATED PATH
