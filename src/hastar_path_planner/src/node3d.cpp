...
bool Node3D::isOnGrid(const int width, const int height) const {
  ...
  (int)((Helper::normalizeHeadingRad(t)+M_PI) / Constants::deltaHeadingRad) >= 0 && 
  (int)((Helper::normalizeHeadingRad(t)+M_PI) / Constants::deltaHeadingRad) < Constants::headings;
}
...
bool Node3D::operator == (const Node3D& rhs) const {
  return (int)(x/Constants::cellSize) == (int)(rhs.x/Constants::cellSize) &&
         (int)(y/Constants::cellSize) == (int)(rhs.y/Constants::cellSize) &&
         (std::abs(Helper::normalizeHeadingRad(t - rhs.t)) < Constants::deltaHeadingRad);
}
