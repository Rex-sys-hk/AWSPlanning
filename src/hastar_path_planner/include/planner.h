...
class Planner {
  ...
  */
  ...
  void CB_getRobotState(const nav_msgs::Odometry& odom);
...
//   std::vector<Node3D*> TracePath(const Node3D* node, int i = 0, std::vector<Node3D*> path = std::vector<Node3D*>());
 ...
 private:
  ...
  Path smoothedPath = Path(Constants::path_smooth);
