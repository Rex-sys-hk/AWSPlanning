...
class CollisionDetection {
  ...
  */
  template<typename T> bool isTraversable(const T* node) const {
    ...
    */
    ...
    if (Constants::online_outline_collision_avoid) {
      cost = configurationOnline(x, y, t) ? 0 : 1;}
    if (Constants::outline_collision_avoid_test) {
      cost = configurationTest(x, y, t) ? 0 : 1;}
    if (Constants::outline_collision_avoid_cost) {
  ...
  */
  ...
  bool configurationOnline(float x, float y, float t) const;
