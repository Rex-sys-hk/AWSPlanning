...
class Path {
 public:
  ...
  Path(bool smoothed = false) {
    ...
    pubPathVel = n.advertise<trajectory_msgs::JointTrajectory>(pathTopic + "_sample_vel", 1);
  ...
  /*!
     ...
     \param i a parameter for counting the number of nodes
  */
  void updatePath(const std::vector<const Node3D*> &nodePath);
  /*!
  ...
  */
  ...
  void publishPathVel(){ pubPathVel.publish(pathVel);}
 ...
 private:
  ...
  nav_msgs::Path path;
  /// IMPORTANT: Velocity  at each node, pose.x is vx, pose.y is vy, pose.z is dyaw
  trajectory_msgs::JointTrajectory pathVel;
